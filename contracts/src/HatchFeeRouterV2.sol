...
/// @title HatchFeeRouterV2
/// @notice PONS creatorFeeRecipient for HATCH. Claims quote-asset fees and splits
///         them deterministically four ways.
///
/// @dev Supersedes HatchFeeRouter, which had no buyback leg. The split is
///      immutable: 50% Nest (NVDA locked forever), 20% buyback-and-lock (HATCH
///      bought off the market and locked forever), 20% HOOKED, 10% team.
contract HatchFeeRouterV2 {
    using SafeTransferLib for address;

    uint256 public constant BPS = 10_000;
    uint256 public constant NEST_BPS = 5_000;
    uint256 public constant BUYBACK_BPS = 2_000;
    ...
    address public immutable buybackLocker;
    ...
    event FeesSplit(
        uint256 total, uint256 nestAmount, uint256 buybackAmount, uint256 hookedAmount, uint256 teamAmount
    );
    ...
    constructor(
        ...
        if (
            quoteToken_ == address(0) || ponsEscrow_ == address(0) || ponsFactory_ == address(0)
                || nest_ == address(0) || buybackLocker_ == address(0) || hookedTreasury_ == address(0)
